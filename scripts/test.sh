...
#  for pred_len in 1 7 14 30 60 180 365
  for pred_len in 365
  do
    echo "Running demand sg${run_name} pl${pred_len}..."
    ...
    accelerate launch --mixed_precision bf16 --num_processes $num_process --main_process_port $master_port --gpu_ids 1 run_main.py \
      ...
      --data_path demand_data_all_cleaned${run_name}.csv \
      --results_path ./results/data${run_name}/ \
      --model_id ${model_name}_Demand_${seq_len}_${pred_len}${run_name} \
      --model $model_name \
      --data Demand \
      --features MS \
      --seq_len $seq_len \
      ...
      --model_comment $comment | tee results/logs/${model_name}-Demand-${pred_len}_batch${batch_size}_dmodel${d_model}${run_name}.txt
  done
done
