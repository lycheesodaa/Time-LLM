...
data_dir = Path('dataset/carbon/res_daily')
results_dir = Path('results/carbon_daily')
results_dir.mkdir(parents=True, exist_ok=True)

df = pd.read_csv(data_dir / 'merged_data_imputed.csv')
...
# pred_lens = [4, 5, 7, 14, 21, 28, 30, 35, 40, 45, 50, 55, 60, 70, 80]
pred_lens = [30, 60, 90]
# pred_lens = [1, 7, 90]
# feat_pct = [0, 0.1, 0.2, 0.3, 0.4]
# feat_pct = [0, 0.1, 0.2]
feat_pct = [0.25, 0.5, 0.75]
feats_map = {}
...
for pct in feat_pct:
    ...
    sel_features_df0 = pd.read_excel("dataset/carbon/res_daily/ranked_abs_features_daily.xlsx")
    ...
    for pl in pred_lens:
        ...
        if CROSS_VAL:
            ...
            log_into_csv(
                ...
                name=f'carbon_daily_{pct}-{num_features}',
                stage='zeroshot',
                model = 'TimeGPT',
                seq_len=-1,
                pred_len=pl,
            ...
            outputs = pd.merge(outputs, df[['Date', 'Price']], on='Date', how='left')
